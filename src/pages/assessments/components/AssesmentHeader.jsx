...
export default function AssessmentHeader() {
  ...
  return (
    <div className="flex items-start justify-between pb-8 mb-6 border-b border-gray-200 bg-gradient-to-r from-amber-50 to-emerald-50 rounded-xl p-6 shadow-sm">
      {/* Left side: title + description */}
      <div className="flex-1 space-y-3">
        <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
          Assessment Builder
          <Badge variant="secondary" className="px-3 py-1 text-xs font-semibold rounded-full bg-emerald-100 text-emerald-700">
            Job ID: {jobId}
          </Badge>
        </h1>
        <p className="text-sm text-gray-600 max-w-lg leading-relaxed">
          Build comprehensive assessments with multiple sections and questions. 
          Or quickly start with a ready-made template below.
        </p>
        ...
        <div className="mt-4 max-w-md">
          <Input
            ...
            onChange={(e) => setTitle(e.target.value)}
            placeholder="e.g. Frontend Developer Assessment"
            className="text-base border border-amber-300 rounded-lg shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"
          />
        </div>
      </div>

      {/* Right side: sample assessment buttons */}
      <div className="ml-8 bg-white border border-amber-200 rounded-xl p-4 shadow-md w-72">
        ...
        <div className="flex flex-col gap-2">
          <Button
            size="sm"
            variant="secondary"
            className="justify-start rounded-md bg-amber-50 hover:bg-amber-100 text-amber-800 transition"
            onClick={() => handleLoadSampleAssessment("frontend")}
          >
            🚀 Frontend Developer
          </Button>
          <Button
            size="sm"
            variant="secondary"
            className="justify-start rounded-md bg-emerald-50 hover:bg-emerald-100 text-emerald-700 transition"
            onClick={() => handleLoadSampleAssessment("backend")}
          >
            ⚙️ Backend Developer
          </Button>
          <Button
            size="sm"
            variant="secondary"
            className="justify-start rounded-md bg-lime-50 hover:bg-lime-100 text-lime-700 transition"
            onClick={() => handleLoadSampleAssessment("general")}
          >
            📘 General Aptitude
          </Button>
